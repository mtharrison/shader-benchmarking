...
pub fn compile_matrix_reduction_pipeline(
    ...
    Ok(CompiledMatrixReductionPipeline {
        ...
        host_launch,
        notes: vec![
            "Stage 1 launches one block per (matrix, column) pair and parallelizes the row reduction across the block.".to_string(),
            format!(
                "Stage 2 launches one block per column and reduces across {matrices} matrices before dividing to form the averaged column sums."
            ),
            format!(
                "Stage 3 launches one block per matrix to reduce {cols} column sums, and stage 4 uses one block to reduce the {matrices} per-matrix totals into a grand total."
            ),
